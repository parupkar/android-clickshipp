...
import com.clickshipp.shop.activities.Login;
import com.clickshipp.shop.adapters.CartItemsAdapter;
import com.clickshipp.shop.adapters.CouponsAdapter;
import com.clickshipp.shop.adapters.DemoCouponsListAdapter;
...
import com.clickshipp.shop.utils.FacebookPixel;
...
public class My_Cart extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        //scratchTextView = rootView.findViewById(R.id.scratch_watcher);
       ...
       /* if (cartItemsList.size() != 0) {
        ...
        cart_coupons_recycler.setLayoutManager(new LinearLayoutManager(getContext(), RecyclerView.VERTICAL, false));
        ...
        // Get scratch Card Functionality.
  /*      scratchTextView.setRevealListener(new ScratchTextView.IRevealListener() {
            @Override
            public void onRevealed(ScratchTextView tv) {
                //on reveal
                Toast.makeText(getContext(),"on reveal",Toast.LENGTH_SHORT).show();
            }
            ...
            public void onRevealPercentChangedListener(ScratchTextView stv, float percent) {
                ...
                Toast.makeText(getContext(),"on text percent reveal",Toast.LENGTH_SHORT).show();
            }
        });*/
        ...
        FacebookPixel.logAddedToCartEvent(logger, getString(R.string.item_added_to_cart), "", "Standerd", ConstantValues.CURRENCY_SYMBOL, cartTotalPrice);
            ...
            for (int i = 0; i < couponsList.size(); i++) {
                ...
                for (int x = 0; x < cartItemsList.size(); x++) {
                    if (couponsList.get(i).getValid_items().contains(cartItemsList.get(x).getCartID())) {
                        ...
                        validItemsCount += cartItemsList.get(x).getCustomersBasketQuantity();
                    }
                ...
                if (!validateCoupon(couponsList.get(i)) || !cartHasValidItemsForCoupon) {
                    ...
                    if (couponsList.get(i).getDiscountType().equalsIgnoreCase("fixed_product")) {
                        for (int x = 0; x < cartItemsList.size(); x++) {
                            if (couponsList.get(i).getValid_items().contains(cartItemsList.get(x).getCartID())) {
                                couponDiscount += (Double.parseDouble(couponsList.get(i).getAmount()) * cartItemsList.get(x).getCustomersBasketQuantity());
                            }
                        }
                    ...
                    for (int x = 0; x < cartItemsList.size(); x++) {
                        if (couponsList.get(i).getValid_items().contains(cartItemsList.get(x).getCartID())) {
                            double totalPrice = Double.parseDouble(cartItemsList.get(x).getTotalPrice()) - (productDiscount * cartItemsList.get(x).getCustomersBasketQuantity());
                            cartItemsList.get(x).setTotalPrice(String.valueOf(totalPrice));
                        }
                    }
    ...
    private void proceedCheckout() {
        ...
        for (int i = 0; i < cartItemsList.size(); i++) {
            ...
            Log.i("variationID", "selectedVariationID = " + cartItemsList.get(i).getSelectedVariationID());
            orderProduct.setId(cartItemsList.get(i).getId());
            orderProduct.setProductId(cartItemsList.get(i).getId());
            orderProduct.setVariationId(cartItemsList.get(i).getSelectedVariationID());
            orderProduct.setQuantity(GetItemQTY(cartItemsList.get(i).getId()));
            orderProduct.setName(cartItemsList.get(i).getName());
            orderProduct.setPrice(cartItemsList.get(i).getPrice());
            orderProduct.setSubtotal("" + Double.parseDouble(cartItemsList.get(i).getPrice()) * GetItemQTY(cartItemsList.get(i).getId()));
            orderProduct.setTotal(cartItemsList.get(i).getTotalPrice());
            orderProduct.setTaxClass(cartItemsList.get(i).getTaxClass());
        ...
        // Save the ProductDetails
        ((App) getContext().getApplicationContext()).setProductDetails(cartItemsList);
    ...
    public static int GetItemQTY(int product_id) {
        User_Cart_DB user_cart_db = new User_Cart_DB();
        return user_cart_db.getUserCartQty
                (
                        product_id
                );
    ...
    public static void DeleteCartItemID(int cart_item_id) {
        User_Cart_DB user_cart_db = new User_Cart_DB();
        user_cart_db.deleteCartItemID
                (
                        cart_item_id
                );
    }
    ...
    public static void ClearCart() {
        ...
        for (int i = 0; i < My_Cart.getCartSize(); i++) {
            My_Cart.DeleteCartItemID(cartItemsList.get(i).getId());
        }
    ...
    public void applyCoupon(CouponDetails coupon) {
        ...
        if (coupon.getDiscountType().equalsIgnoreCase("fixed_product")) {
            ...
            for (int i = 0; i < cartItemsList.size(); i++) {
                if (cartItemsList.get(i).isProductValidForCoupon()) {
                    validItemsCount += GetItemQTY(cartItemsList.get(i).getId());
                    couponDiscount += (Double.parseDouble(coupon.getAmount()) * validItemsCount);
                }
            }
        ...
        if ((couponDiscount + cartDiscount) >= cartSubTotal) {
            ...
            for (int i = 0; i < cartItemsList.size(); i++) {
                if (cartItemsList.get(i).isProductValidForCoupon()) {
                    if (0 > Double.parseDouble(cartItemsList.get(i).getTotalPrice()) - productDiscount) {
    ...
    private boolean validateCoupon(CouponDetails coupon) {
        ...
        for (int i = 0; i < cartItemsList.size(); i++) {
            ...
            int productID = cartItemsList.get(i).getId();
            List<String> categoryIDs = new ArrayList<>();
            if (!"".equalsIgnoreCase(cartItemsList.get(i).getCategoryIDs()) && cartItemsList.get(i).getCategoryIDs() != null)
                categoryIDs = Arrays.asList(cartItemsList.get(i).getCategoryIDs().replaceAll("\\s", "").split(","));
            ...
            if (coupon.isExcludeSaleItems()) {
                if (!cartItemsList.get(i).isOnSale()) {
