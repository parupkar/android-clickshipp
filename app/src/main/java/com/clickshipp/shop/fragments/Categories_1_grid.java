...
import com.clickshipp.shop.adapters.CategoryListAdapter_1;
...
public class Categories_1_grid extends Fragment {
    ...
    List<CategoryDetails> sixCategoriesList;
...
//            ((MainActivity)getActivity()).toggleNavigaiton(false);
        ...
        if (mainCategoriesList.size() >= 6){
            sixCategoriesList = new ArrayList<>();
            for (int i=0; i<6; i++) {
                sixCategoriesList.add(mainCategoriesList.get(i));
            }
        } else {
            sixCategoriesList = new ArrayList<>();
        ...
        categoryListAdapter = new CategoryListAdapter_1(getContext(), sixCategoriesList, false);
